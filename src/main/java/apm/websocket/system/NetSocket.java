...
@ServerEndpoint(value = "/netSocket", encoders = {NetEncoder.class})
public class NetSocket {
	...
	private static CopyOnWriteArraySet<NetSocket> webSocketSet = new CopyOnWriteArraySet<NetSocket>();
	...
	public void sendMessage(Session session) throws IOException, EncodeException, SigarException {
		if (session.isOpen()) {
			NetEntity net = getNetInfo();
			if (net != null) {
				session.getBasicRemote().sendObject(net);
			}
		}
	}

	private NetEntity getNetInfo() throws SigarException {
		...
		NetEntity net = new NetEntity(ifstat.getRxPackets(), ifstat.getRxBytes(), ifstat.getRxErrors(),
				ifstat.getRxDropped(), ifstat.getTxPackets(), ifstat.getTxBytes(), ifstat.getTxErrors(),
				ifstat.getTxDropped());
		sigar.close();
		return net;
	}
