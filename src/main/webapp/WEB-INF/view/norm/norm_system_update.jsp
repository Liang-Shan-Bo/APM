			...
			<div class="main-content">
				...
				<div class="page-content">
					<div class="page-header">
						<h1>
							系统指标<small> <i class="icon-double-angle-right"></i> 编辑
							</small>
						</h1>
					</div>
					<!-- 编辑指标 -->
						...
						<div class="col-xs-12">
							<form id="updateForm" class="form-horizontal" role="form" action="<%=path%>/updateNorm" method="post">
								...
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="text" id="normName" class="col-xs-10 col-sm-4" name="normName" value="${normEntity.normName}" readonly="readonly"/>
									</div>
								</div>
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right" for="normNormal"> 正常负载
										<c:if test="${normEntity.normType == 1 or normEntity.normType == 3}">(%)</c:if>
										<c:if test="${normEntity.normType == 2}">(M)</c:if>
										<c:if test="${normEntity.normType == 4}">(KB)</c:if>
									</label>
									<div class="col-sm-9">
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right" for="normWarning"> 警告负载 
										<c:if test="${normEntity.normType == 1 or normEntity.normType == 3}">(%)</c:if>
										<c:if test="${normEntity.normType == 2}">(M)</c:if>
										<c:if test="${normEntity.normType == 4}">(KB)</c:if>
									</label>
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right" for="normDanger"> 过高负载 
										<c:if test="${normEntity.normType == 1 or normEntity.normType == 3}">(%)</c:if>
										<c:if test="${normEntity.normType == 2}">(M)</c:if>
										<c:if test="${normEntity.normType == 4}">(KB)</c:if>
									</label>
